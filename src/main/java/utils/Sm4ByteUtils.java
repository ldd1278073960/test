...
public class Sm4ByteUtils {
    ...
    protected void sm4_setkey_enc(Sm4ContextUtils ctx, byte[] key) throws Exception {
        if (ctx == null) {
    ...
    protected void sm4_setkey_dec(Sm4ContextUtils ctx, byte[] key) throws Exception {
        if (ctx == null) {
            throw new Exception("ctx is null!");
        } else if (key != null && key.length == 16) {
            ctx.mode = 0;
    ...
    protected byte[] sm4_crypt_ecb(Sm4ContextUtils ctx, byte[] input) throws Exception {
        if (input == null) {
